...
bool ofxIpCamera::grabFrame() {
  ...
  //  this was moved to do it just once at he beginning of threadedFunction
...
void ofxIpCamera::setupSession(Poco::Net::HTTPClientSession& theClient) {
    theClient.setHost(uri.getHost());
    if(uri.getPort() != 0)
    	theClient.setPort(uri.getPort());

    theClient.setKeepAlive(true); // use a persistent connection while running
    theClient.setTimeout(Poco::Timespan(5, 0)); // s, ms; this is live, dont wait too long for a response
}
...
bool ofxIpCamera::grabOneFrame(Poco::Net::HTTPClientSession& client) {
    ...
    timeLapse = currMillis-lastMillis;

    if (timeLapse < MILLIS_PER_FRAME) return true;

    bool bNewResponse = ((responseBody == NULL) || (responseBody->eof()));
    bool bGotJPG = false;

	if (bNewResponse) {
	  //  if ( (responseBody->eof()) && (responseBody != NULL)) free(responseBody);
        // send request
        ...
        contentType.assign(response.getContentType());
        if (contentType.find("image/jpeg") != string::npos) {
               bGotJPG = getJpgFromBuffer(response.getContentLength());
               bIsBufferEmpty = responseBody->eof();
               lastMillis = currMillis;
        }
	}

    if (contentType.find("multipart/x-mixed-replace") != string::npos) {
        // look for first image/jpeg content between boundaries
        string boundary_id("boundary=");
        // string boundary(content_type, content_type.find(boundary_id)+boundary_id.length(), 256);
        string boundary(contentType, contentType.find(boundary_id)+boundary_id.length(), 256);
        string content_type_str("Content-Type: ");
        string content_length_str("Content-Length: ");
        size_t found;


        // as long as there are frames available in the response's body
        while (!responseBody->eof() && (timeLapse >= MILLIS_PER_FRAME)) {
             // find beginning boundary
            string boundary_line;
            while ((!responseBody->eof()) && ( (found = boundary_line.find(boundary)) ==string::npos)) {
                getline(*responseBody, boundary_line);
            }
            //  cout << boundary_line << "1 \n";
            // Content-Type: image/jpg
            string type_line;
            while ((!responseBody->eof()) && ( (found = type_line.find(content_type_str)) == string::npos)) {
                getline(*responseBody, type_line);
            }
            string contentType2(type_line, found+content_type_str.length(), 256);
            //cout << "content: " << contentType2 << "\n";
            if (contentType2.find("image/jpeg") == string::npos)
            {
                ofLog(OF_LOG_ERROR, "ofxIpCamera: Received content type is not \"image/jpeg\"");
                ofLog(OF_LOG_ERROR, contentType);
                return false;
            }
            // Content-Length:
              string length_line;
            while ((!responseBody->eof()) && ( (found = length_line.find(content_length_str)) ==string::npos)) {
                getline(*responseBody, length_line);
            }

            string contentLength(length_line, found+content_length_str.length(), 256);
            //cout << "length: " << contentLength << "\n";
            unsigned int bufferSize = atoi(contentLength.c_str());
            // skip empty lines (lines that contain only the '\n' character
            string skip("something");
            while (!responseBody->eof()  && (skip.length() > 1)) {
                 getline(*responseBody, skip);
                // cout << skip << "\n";
            }

           // if (!bNeedsUpdate) {
                bGotJPG = getJpgFromBuffer(bufferSize);
          //  }
            bIsBufferEmpty = responseBody->eof();
        ...
        ofLog(OF_LOG_ERROR, "ofxIpCamera: Received content type is not \"image/jpeg\" nor \"multipart/x-mixed-replace\" ");
        ofLog(OF_LOG_ERROR, contentType);
        return false;
    }
    //
...
bool ofxIpCamera::getJpgFromBuffer(int bufferSize) {
            Poco::Buffer<uint8_t> buffer(bufferSize);
            responseBody->read((char*)buffer.begin(), bufferSize);

            // decode and load image from raw memory
            FIMEMORY* imageMem = FreeImage_OpenMemory(buffer.begin(), bufferSize);
            if(imageMem == NULL) {
...
bool ofxIpCamera::sendRequest(string theRequest)
{
    Poco::Net::HTTPClientSession httpClient;
    Poco::Net::HTTPRequest request;

    setupSession(httpClient);
    request.setVersion(Poco::Net::HTTPMessage::HTTP_1_1);
    request.setURI(theRequest);
    credentials.authenticate(request);
...
bool ofxIpCamera::sendRequest(string theRequest, string &theResponse)
{
    // setup http session
    Poco::Net::HTTPClientSession client2;
    Poco::Net::HTTPRequest request;

    setupSession(client2);
    request.setVersion(Poco::Net::HTTPMessage::HTTP_1_1);
    request.setURI(theRequest);
    credentials.authenticate(request);
  ...
  //  this->lock();
    ...
    // receive response
    Poco::Net::HTTPResponse response;
    istream* theResponseBody;

    try {
        theResponseBody = (istream*) &client2.receiveResponse(response);
    }
    catch(Poco::Net::NoMessageException& e) {
        // catch no message exception when shutting down thread
        return false;
    ...
    if (!responseBody->eof()) {
        getline(*theResponseBody, theResponse);
    }
    return true;
}
